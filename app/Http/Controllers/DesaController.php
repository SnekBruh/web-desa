    ...
    public function update(Request $request, Desa $desa)
    {
        if (request()->ajax()) {
            ...
            $data = $request->validate([
                ...
                'visi'                  => ['required', 'max:255', 'string'],
                'misi'                  => ['required', 'max:255', 'string'],
            ]);

            if ($request->nama_desa != $desa->nama_desa  || $request->nama_kecamatan != $desa->nama_kecamatan || $request->nama_kabupaten != $desa->nama_kabupaten || $request->alamat != $desa->alamat || $request->nama_kepala_desa != $desa->nama_kepala_desa || $request->alamat_kepala_desa != $desa->alamat_kepala_desa || $request->visi != $desa->visi || $request->misi != $desa->misi) {
                $desa->update($data);
